...
def piece(request, pk):
    ...
    return render(request, "production/piece.html", {"piece": item})
def etape(request,pk,statut):
    ...
    if item.operateur_id and item.operateur_id != request.user.id:
        raise PermissionDenied
    if request.method=="POST" and statut in dict(EtapeProduction.Statut.choices):
        try:
            changer_etape(item,statut,request.user,request.POST.get("commentaire",""),request.POST.get("probleme",""))
